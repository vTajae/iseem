...
# at https://dashboard.plaid.com/team/app.api
...
def get_assets():
    ...
    if asset_report_json is None:
        return JSONResponse({'error': {'status_code': err.status, 'display_message':
...
def transfer():
    ...
    try:
        ...
        pretty_print_response(response.to_dict())
        return JSONResponse({'error': None, 'transfer': response['transfer'].to_dict()})
    except plaid.ApiException as e:
        error_response = format_error(e)
        return JSONResponse(error_response)
